<template>
    ...
    <el-collapse-item name="2">
        ...
        <!-- 副标题内容 -->
        <chart-base-input :inputValue.sync="subTitle.text" :placeholder="'请输入副标题内容'">
            <div slot="input">副标题内容</div>
        </chart-base-input>
        ...
        <chart-base-select :selectOption="distanceOption" :selectValue.sync="subTitle.distance.value">
            <div slot="select">主副标题间距</div>
        </chart-base-select>

        <!-- 自定义间距 -->
        <chart-base-slider v-if="subTitle.distance.value === 'custom'" :baseSliderOption.sync="subTitle.distance.cusGap" :unit="'px'" :content="'滑动修改间距'"></chart-base-slider> 
    </el-collapse-item>
</template>
